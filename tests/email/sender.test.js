...
const hoisted = vi.hoisted(() => ({ mockFetch: vi.fn() }));

vi.stubGlobal('fetch', hoisted.mockFetch);

const { mockFetch } = hoisted;
...
describe('sendCampaign', () => {
  beforeEach(() => {
    ...
    // Default ML API sequence: GET /groups → POST /groups → GET /campaigns (none) → POST /campaigns → POST /schedule
    mockFetch
  ...
  it('sendCampaign_returnsCampaignIdAndNullRecipientCount', async () => {
    const result = await sendCampaign('<html>test</html>', 'Subject');

    expect(result.campaignId).toBe('camp-1');
    expect(result.recipientCount).toBeNull();
  });
  ...
  it('sendCampaign_reusesExistingCampaignOnRetry', async () => {
    ...
    mockFetch.mockReset();
    // Code order: GET /groups → GET /campaigns → POST /campaigns → POST /schedule
    mockFetch
      // attempt 1

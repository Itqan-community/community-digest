#!/usr/bin/env bash
# provision.sh — create the community-digest droplet in the same DO VPC as flarum-prod-db
...
# 2. Resolve VPC from flarum-prod-db (so we land in the same private network)
echo "Resolving VPC from $REFERENCE_DROPLET..."
...
# 4. Create droplet
echo "Creating droplet '$DROPLET_NAME' ($SIZE, $REGION, VPC $VPC_UUID)..."
...
for i in $(seq 1 30); do
  if ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 \
      -i ~/.ssh/id_rsa "root@$PUBLIC_IP" "echo ok" &>/dev/null; then
    echo "  SSH ready."
    break
  fi
  echo "  Attempt $i/30..."
